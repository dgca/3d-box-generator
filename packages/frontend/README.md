...
Create a production build from the workspace root:

```bash
pnpm build
```

Run linting and type checks from the workspace root:
...
The UI consumes the `3d-box-generator` workspace package, which can generate meshes or STL strings without touching React components.
...
- The core API is packaged in the workspace, but it has not been published to npm yet
